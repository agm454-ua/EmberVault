...
export default function ShareResourceModal({ resource, onClose }: { resource: TResourceResponse; onClose: () => void }) {
    ...
    const assignedUsers = useMemo(() => {
        return ((usersWithRole.data as ExistingAssignment[] | undefined) ?? [])
            .map((assignment) => ({
            ...
            <div className="bg-surface-canvas rounded-lg p-6 w-120 max-w-[95vw] flex flex-col gap-4">
                ...
                {normalizedQuery.length > 0 && (
                    ...
                    <div className="max-h-56 overflow-y-auto border border-stroke-muted rounded-md p-2">
                        {searchUsers.isFetching ? <p className="text-sm text-ink-muted px-2 py-3">{t('nav.loading')}</p> : null}

                        {showNoResults ? (
                            <p className="text-sm text-ink-muted px-2 py-3">{t('search.noResults', { query: normalizedQuery })}</p>
                        ) : null}
                                ...
                                (
                                    ...
                                    >
                                        <div className="min-w-0">
                                            ...
                                            <p className="text-xs text-primary-600 capitalize">{t('userData.role')}: {entry.roleName}</p>
                                        </div>
                                        <Button
                                            variant="danger"
                ...
                <div className="flex justify-end gap-2 pt-2">
                    ...
                    <Button
                        ...
                        disabled={!selectedUser || !selectedRoleName || isRoleAlreadyAssigned || setUserResourceRole.isPending}
                    >
                        {t('actions.share')}
                    </Button>
                </div>
